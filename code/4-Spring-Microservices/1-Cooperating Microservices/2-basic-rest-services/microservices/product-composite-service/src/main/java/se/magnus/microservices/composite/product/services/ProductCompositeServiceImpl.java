...
public class ProductCompositeServiceImpl implements ProductCompositeService {
  ...
  public ProductAggregate getProduct(int productId) {
    ...
    Product product = integration.getProduct(productId); // sync / blocking IO
    if (product == null) {
      throw new NotFoundException("No product found for productId: " + productId);
    }

    List<Recommendation> recommendations = integration.getRecommendations(productId); // sync

    List<Review> reviews = integration.getReviews(productId); // sync / blocking IO
